...
void set_log_level(const char* new_level) {
  ...
  else if (strcmp("PROFILE", new_level) == 0) {
    log_level = PROFILE;
    log_printf(INFO, "Logging level set to PROFILE");
  }
...
void log_set_ranks(MPI_Comm comm) {
  _MPI_comm = comm;
  _MPI_present = true;
  MPI_Comm_rank(_MPI_comm, &log_rank);
}
#endif
